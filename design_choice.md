...
- **Independent Baselines & Shared Preservation**: 
  - The **Hair Zone** (Foreground) compares the Edited Image to the Style Reference Image.
  - The **Background Zone** (Inverse Mask) compares the Edited Image to the Original Image. To prevent spatial mismatch (e.g., if the generated hair expands and covers more of the background), the pipeline computes the intersection (`cv2.bitwise_and`) of the Original Background Mask and the Edited Background Mask. This creates a "Shared Preservation Zone", guaranteeing that features are extracted from the exact same spatial pixels across both images.
